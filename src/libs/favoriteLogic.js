import { isPostSaved, removePost, savePost } from "./logic.js";
  ...
  if (saveButtons.length > 0) {
   saveButtons.forEach((button) => {
    ...
    button.addEventListener("click", (event) => {
     ...
     const postFormat = event.currentTarget.getAttribute("data-format");
     const postSize = event.currentTarget.getAttribute("data-size");
     const deletedBanner = document.querySelector(".banner.deleted");
     const addedBanner = document.querySelector(".banner.added");
     ...
     if (isPostSaved(postId)) {
      ...
      savePost(postId, postTitle, postFormat, postSize);
      event.currentTarget.innerHTML = deleteIcon;
      addedBanner.style.display = "block";
      setTimeout(() => {
       addedBanner.style.display = "none";
      }, 2000);
     }
    });
   });
